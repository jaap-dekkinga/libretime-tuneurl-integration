...
from .analyze_replaygain import analyze_replaygain, concate_sound
...
class Pipeline:
    ...
    def concate_sound(
        ...
        try:
            ...
            if not isinstance(trigger_sound_path, str):
                raise TypeError(
                    "trigger_sound_path must be unicode. Was of type "
                    + type(trigger_sound_path).__name__
                    + " instead."
                )
            ...
            metadata = {}
            # metadata = analyze_metadata(audio_file_path, metadata)
            logger.info("pipeline log 111 %s ", trigger_sound_path)
            metadata = concate_sound(trigger_sound_path, audio_file_path)
            # metadata["import_status"] = PipelineStatus.SUCCEED
            logger.info("pipeline log 222 %s ", audio_file_path)
            # Pass all the file metadata back to the main analyzer process
            # queue.put(metadata)
            return True
        except UnplayableFileError as exception:
            logger.exception(exception)
            # metadata["import_status"] = PipelineStatus.FAILED
            # metadata["reason"] = "The file could not be merged."
            raise exception
        except Exception as exception:
            logger.exception(exception)
            raise exception
