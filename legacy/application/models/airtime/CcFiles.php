...
class CcFiles extends BaseCcFiles
{
    ...
    public static function concateTriggerSound($targetFileName)
    {
        if (Application_Model_Systemstatus::isDiskOverQuota()) {
            throw new OverDiskQuotaException();
        }
        $triggerFilePath = Config::getStoragePath() .'imported/Trigger-Audio.mp3';
        $audioTrackPath = Config::getStoragePath() .'imported/'. self::getOwnerId() .'/'.$targetFileName;
        // Command to merge audio files
        // $command = "ffmpeg -i \"concat:$file1|$file2\" -acodec copy $file2";
        
        Application_Service_MediaService::notifiyAnalyzer(
            $triggerFilePath,
            $audioTrackPath
        );
