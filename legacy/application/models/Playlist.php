...
SQL;
    ...
    private function insertPlaylistElement($info)
    {
        ...
        if(isset($info['tuneurl_id']) && $info['tuneurl_id'] !== null){
            $row->setDbTuneurlId($info['tuneurl_id']);
        }
    ...
    private function buildEntry($p_item, $pos,  $tuneurlId=null)
    {
        $objType = $p_item[1];
        $objId = $p_item[0];
     ...
     */
    public function addAudioClips($p_items, $p_afterItem = null, $addType = 'after', $tuneurlId = null)
    {
        ...
        try {
            ...
            foreach ($p_items as $ac) {
                $res = $this->insertPlaylistElement($this->buildEntry($ac, $pos,  $tuneurlId));
